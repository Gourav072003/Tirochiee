    ...
    <div class="card-body">
      <form method="post" action="{{route('product.update',$product->id)}}">
        ...
        <div class="form-group">
          ...
          <input id="thumbnail" class="form-control" type="text" name="photo" value="{{$product->photo}}" placeholder="Select one or more product images">
        </div>
        <small class="form-text text-muted">Choose multiple images for the same product gallery. Existing images stay comma-separated in this field.</small>
...
<script>
    ...
    $(document).ready(function() {
      const holder = $('#holder');
      const images = ($('#thumbnail').val() || '').split(',').map(item => item.trim()).filter(Boolean);

      holder.html('');
      images.forEach(function(image) {
        holder.append('<img src="' + image + '" style="height:5rem;margin-right:10px;margin-bottom:10px;">');
      });
    });

    $('#thumbnail').on('change', function() {
      const holder = $('#holder');
      const images = ($(this).val() || '').split(',').map(item => item.trim()).filter(Boolean);
